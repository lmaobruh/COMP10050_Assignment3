	...
	*downRight = &board[boardSize -1][boardSize -1];
	...
	for(int i =0; i< boardSize; i++){
			...
			for(int j=0;j < boardSize; j++){
				...
				if(rng == 1){ // if rng is greater than rng1 and rng2 slot[i].slotType will get assigned city type.
...
struct slot * reachDesiredElement(int row, int column, struct slot * initialSlot){
	...
	while(found == false){
		...
		if(currentSlot->row > row){
			...
			//printf("Current slot (%d, %d, %s) -> \n",currentSlot->row,currentSlot->column,currentSlot->slotType);
		}
